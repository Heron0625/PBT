...
class WinCoTransLayer(nn.Module):
    def __init__(self,
        ...
        self.attn_proj = nn.ConvTranspose2d(in_channels=2*dim, out_channels=dim,kernel_size=2,stride=2,padding=0) \
            if self.mode == 'cross' else nn.Identity()
    ...
    def forward(self, x, attn_kv=None):
        ...
        # DENIED
        if self.mode == 'cross':
            attn_kv = bchw_to_blc(self.attn_proj(attn_kv))
...
class WindowCoTransEncoder(nn.Module):
    def __init__(self, dim, mode, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        self.norm2 = norm_layer(dim // downsample) if mode == 'cross' else nn.Identity()
...
class WindowPBTNet(nn.Module):
    def __init__(self, num_classes, input_channels, num_blocks, nb_filter, deep_supervision,
        ...
        self.attn0_1 = WinCoTransLayer(sigma_dim=nb_filter[0] + nb_filter[1], dim=nb_filter[0], depth=depth[0],
                                       ...
                                       win_size=win_size, input_resolution=img_size, shift_size=win_size//4)
        ...
        self.attn3_1 = WinCoTransLayer(sigma_dim=nb_filter[2] + nb_filter[3] + nb_filter[4],mode='cross',
        ...
        self.attn2_2 = WinCoTransLayer(sigma_dim=nb_filter[1] + nb_filter[2] * 2 + nb_filter[3], dim=nb_filter[2],mode='cross',
        ...
        self.attn1_3 = WinCoTransLayer(sigma_dim=nb_filter[0] + nb_filter[1] * 3 + nb_filter[2], dim=nb_filter[1],mode='cross',
        ...
        self.attn0_4 = WinCoTransLayer(sigma_dim=nb_filter[0] * 4 + nb_filter[1], dim=nb_filter[0],mode='cross',
    ...
    def forward(self, input,return_attn = None):
        ...
        x3_1 = self.attn3_1(torch.cat([x3_0, self.up(x4_0), self.down(x2_0)], 1), attn_kv=self.attn_kv_transform(x4_0))# x4_1
        ...
        x2_2 = self.attn2_2(torch.cat([x2_0, x2_1, self.up(x3_1), self.down(x1_1)], 1), attn_kv=self.attn_kv_transform(x3_1))# x3_2
        ...
        x1_3 = self.attn1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2), self.down(x0_2)], 1), attn_kv=self.attn_kv_transform(x2_2))# x2_3
        x0_3 = self.attn0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1), attn_kv=self.attn_kv_transform(x1_3))# x1_3

        x0_4 = self.attn0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3)], 1), attn_kv=self.attn_kv_transform(x1_3))# x1_4
